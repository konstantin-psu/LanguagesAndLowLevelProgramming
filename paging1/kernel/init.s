...
entry: cli # Turn off interrupts
 ...
 #------------------------------------------------------------------
 # Create initial page directory in which the initial ((32)<<20)
...
1: movl %eax, (%edi)
 movl %eax, (4*(KERNEL_SPACE>>22))(%edi)
...
initIDT:# Add descriptors for exception & interrupt handlers:
 ...
 idtcalc handler=syscall, slot=128, dpl=3
 idtcalc handler=putchar, slot=129, dpl=3
 idtcalc handler=ccls, slot=130, dpl=3
 idtcalc handler=setAttr, slot=131, dpl=3
 idtcalc handler=handlecall, slot=132, dpl=3
...
excepted:
 ...
 movb $\info, %al # ICW3: master/slave info
 outb %al, $(\port+1)
...
#--------------------------------------------------------------------------
 ...
 .text
syscall:subl $4, %esp # Fake an error code
 push %gs # Save segments
 push %fs
 push %es
 push %ds
...
putchar:subl $4, %esp # Fake an error code
 push %gs # Save segments
 push %fs
 push %es
 push %ds
...
setAttr:subl $4, %esp # Fake an error code
 push %gs # Save segments
 push %fs
 push %es
 push %ds
